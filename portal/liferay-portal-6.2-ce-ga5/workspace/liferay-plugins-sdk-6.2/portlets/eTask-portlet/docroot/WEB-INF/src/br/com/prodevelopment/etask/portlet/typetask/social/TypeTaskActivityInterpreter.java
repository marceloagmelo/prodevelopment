package br.com.prodevelopment.etask.portlet.typetask.social;
...
public class TypeTaskActivityInterpreter extends BaseSocialActivityInterpreter {
	...
	protected String getPath(SocialActivity activity,
		...
		PortletURL portletURL = PortletURLFactoryUtil.create(
				serviceContext.getRequest(), TypeTaskConstants.PORTLET_NAME, plid,
		...
		portletURL.setParameter(TypeTaskConstants.TYPE_TASK_ID, String.valueOf(activity.getClassPK()));
		portletURL.setParameter(TypeTaskConstants.ACTION, ActionKeys.VIEW);
	...
	protected String getTitlePattern(String groupName, SocialActivity activity) {
		...
		if (activityType == TypeTaskActivityKeys.ADD_TYPE) {
			if (Validator.isNull(groupName)) {
				return "activity-typeticket-add";
			} else {
				return "activity-typeticket-add-in";
			}
		} else if (activityType == TypeTaskActivityKeys.UPDATE_TYPE) {
			if (Validator.isNull(groupName)) {
		...
		} else if (activityType == TypeTaskActivityKeys.ADD_TYPE_USER) {
			if (Validator.isNull(groupName)) {
				return "activity-typeticketusers-update";
			} else {
				return "activity-typeticketusers-update-in";
			}
		} else if (activityType == TypeTaskActivityKeys.DELETE_TYPE_USER) {
			if (Validator.isNull(groupName)) {
	...
	protected boolean hasPermissions(PermissionChecker permissionChecker,
		...
		boolean permissaoView = permissionChecker.hasPermission(
				activity.getGroupId(), TypeTask.class.getName(),
	...
	private static final String[] _CLASS_NAMES = { TypeTask.class.getName() };
}
