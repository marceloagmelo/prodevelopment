...
function marqueiCheck(checkId) {
	...
	if(count > max) {
		count--;
		alert("Não é permitido associar mais do que "+max+" produtos");
	}
}
...
<form action="<%=configuracaoURL%>" method="post">
	...
	<input type="hidden" name="produtosOrdemFromRequest" id="produtosOrdemFromRequest" value='${produtosOrdemFromRequest}'/>

	<fieldset>
	<legend>
		<liferay-ui:message key="escolha-dos-produtos" />
	</legend>
	...
	<div style="border-bottom:1px solid gray; margin-top:4px;margin-bottom:4px;"></div>
		...
		<%
			List<ProdutoVO> produtos = (List<ProdutoVO>) request.getAttribute("produtos");
			if (produtos == null) {
		...
		<liferay-ui:search-container
			...
			<liferay-ui:search-container-results >
				<%
					results = ListUtil.subList(produtos,
			...
			<liferay-ui:search-container-row className="br.com.rasecmadeiras.produto.vo.ProdutoVO" keyProperty="id" modelVar="produto">
			<liferay-ui:search-container-column-text name="exibir">

			<c:set var="id" value="${produto.id}"/>
			<c:set var="idss" value="${meusProdutosIdString}"/>
				<c:choose>
				...
				<input onclick="javascript:ckeckChange('${produto.titulo}','${produto.id}',this.id);" id="${produto.id}" type="checkbox" name="${produto.id}" ${ch}>
		...
		<table>
			<tr>
				<td><select id="layoutIdsBox" name="layoutIdsBox" size="7" multiple="multiple">
						<c:forEach var="produto" items="${meusProdutos}">
