...
<form action="<%=configuracaoURL%>" method="post" name="fm">
	...
	<table border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
			<td><liferay-ui:message key="label.pref.urlVerTodos" />
				<p>
					<input type="text" name="<%=Constantes.PREF_URL_VER_TODOS%>"
						value="${prefUrlVerTodos }" size="100" />
				</p></td>
		</tr>
		<tr>
			...
			<td><liferay-ui:message key="label.pref.estrutura" />
				<p>
					<select name="<%=Constantes.PREF_ESTRUTURA%>">
						<option value="" selected="selected"><liferay-ui:message key="label.todas" /></option>
						<%
						...
						%>
						<option value="<%=estrutura.getStructureId()%>" selected="selected"><%=estrutura.getName()%></option>
						<%
							} else {
						%>
