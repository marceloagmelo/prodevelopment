...
import br.com.prodevelopment.meloesantos.portlet.orcamento.util.Helper;
import br.com.prodevelopment.meloesantos.portlet.orcamento.util.OrcamentoUtil;
import br.com.prodevelopment.meloesantos.portlet.orcamento.validacao.OrcamentoValidator;
...
public class OrcamentoPortlet extends MVCPortlet {
	...
	public void doView(RenderRequest req, RenderResponse res)
		...
		try {
			...
			if (Constantes.S.equals(prefConfigurado)) {
				...
				String portletTitulo = Helper.getTituloPortlet(req,
						res, themeDisplay);

				req.setAttribute(Constantes.PORTLET_TITULO, portletTitulo);

				List<String> listaEstados = OrcamentoUtil.montarListaEstados();
				req.setAttribute(Constantes.LISTA_ESTADOS, listaEstados);
				List<String> listaTipoResposta = OrcamentoUtil.montarListaTipoResposta();
	...
	public void enviar(ActionRequest request, ActionResponse response)
		...
		String remetente = String.valueOf(request.getPreferences()
				.getValue(Constantes.PREF_REMETENTE, Constantes.VAZIO));
		...
		orcamentoVO.setRemetente(remetente);
		orcamentoVO.setNomeUsuario(nomeUsuario);

		// Recuperar usuário
		String usuarioLogado = (OrcamentoUtil.getDadosPortal(request)
				.getUser().isDefaultUser() ? Constantes.ZERO : Constantes.UM);
		if (Constantes.UM.equals(usuarioLogado)) {
			orcamentoVO.setEmailUsuario(OrcamentoUtil
		...
		if (OrcamentoValidator.validar(orcamentoVO, errors)) {
			OrcamentoUtil.enviar(orcamentoVO, resourceBundle, PortalUtil.getLocale(httpReq));
